...
| Statut | Brouillon |
| Date | 2026-09-25 |
| Dépend de | [D2](D2-regles-jeu-arbitrage.md) ; [D3](D3-plan-de-tests.md) ; [D4](D4-architecture-technique.md) ; [D5](D5-parcours-maquettes.md) ; [source de cadrage](../sources/cadrage-lots-1-2-3.md) §3.7, §4.1 ; [D8](D8-journal-decisions.md) n° 35, 44, 52 à 54, 70, 77, 82, 134 à 147 |
| Utilisé par | [D3](D3-plan-de-tests.md) (renvois aux lots) |

## 1. Conventions

- Un **lot de développement** (L0.1, L1.2…) se livre et se teste en **une séance** de travail (n° 44 : effort faible = une séance). Si un lot déborde, il est coupé en deux avant de commencer, pas pendant.
...
- Aucun lot ne stocke d'image ni de son (n° 26). Seul le journal numérique de test (n° 73) est écrit, sur l'ordinateur de Valentin.
...
flowchart LR
    ...
    subgraph P2["P2 — duel complet"]
        L21["L2.1 Déroulé"] --> L22["L2.2 Arbitrage réparti"]
        L22 --> L23["L2.3 Preuve, fin, revanche"]
        L21 --> L24["L2.4 Écrans et erreurs"]
...
| L2.1 | P2 | Dérouler le match selon la machine à états | Interface, Arbitrage | §3.3 |
...
| Rubrique | Contenu |
|---|---|
| Tâches | Page installable (manifeste, service worker minimal) ; accès caméra avec la caméra frontale ; vidéo en miroir ; bibliothèque MediaPipe, modèle et WebAssembly servis par l'hébergement de la PWA ([D4](D4-architecture-technique.md) §1, principe 5) ; mise en ligne en HTTPS |
| Modules | Interface, Capture, Hébergement |
| Terminé quand | La page affiche la caméra sur les trois appareils de test ([D3](D3-plan-de-tests.md) §1.2.3), et l'onglet Réseau du navigateur ne montre aucune requête vers un autre domaine |
...
| Rubrique | Contenu |
|---|---|
| Tâches | Séquences minutées A0 à A7, B1 à B3, C, PERF avec consigne et chronomètre ; touche « sourire vu » ; revue après chaque sourire (confirmée, faux positif, litigieuse) ; journal CSV ([D3](D3-plan-de-tests.md) §1.4.1) ; pic soutenu `P` en fin de séquence |
| Modules | Interface |
| Terminé quand | Une session à blanc de 30 min (Valentin seul) produit un journal complet et lisible au tableur |
...
| Rubrique | Contenu |
|---|---|
| Tâches | Code de salon aléatoire ; lien d'invitation ; ouverture et entrée dans le salon par le service de mise en relation retenu ([D4](D4-architecture-technique.md) §8) ; erreurs « lien plus valable » ; verrouillage à deux ; expiration à 15 min |
...
| Rubrique | Contenu |
|---|---|
| Tâches | Connexion WebRTC audio et vidéo ; serveurs STUN et TURN (UDP, TCP, TLS sur 443) ; option « relais forcé » pour le test ; journal P1 : type de candidat, temps d'établissement, aller-retour, débit, pertes |
| Modules | WebRTC |
...
| Rubrique | Contenu |
|---|---|
| Tâches | Détection de L0.2 active pendant l'appel ; un seul flux caméra partagé ; cadence mesurée par fenêtre de 10 s |
| Modules | Détection, WebRTC |
| Terminé quand | Cadence journalisée pendant 10 min d'appel réel sur l'appareil le plus ancien |
...
| Rubrique | Contenu |
|---|---|
| Tâches | Machine à états de [D2](D2-regles-jeu-arbitrage.md) §6 : accueil et case d'âge, autorisation, calibrage des deux joueurs, écran noir, cadence commune, compte à rebours calé sur t0, manche de 60 s, décision, arrêt sur image, score, 2 manches gagnantes |
...
| Rubrique | Contenu |
|---|---|
| Tâches | Écrans E1 à E11 et ER1 à ER14 de [D5](D5-parcours-maquettes.md), textes exacts ; mise en page portrait et paysage ([D4](D4-architecture-technique.md) §10) ; maintien de l'écran allumé |
| Modules | Interface |
...
| Dépend de | L2.4. Obligatoire avant que le lien circule hors du cercle proche (Priorité 3) |
...
| P1 — horloges (C3) | Revoir `e` ou le multiplicateur ([D2](D2-regles-jeu-arbitrage.md) Q6) | L1.4 | Aucun |
...
|---|---|---|
| Q1 | Durée d'une « séance » : une demi-journée, une soirée ? Le découpage suppose environ 3 à 4 heures. | 3 à 4 heures, hypothèse appliquée |
| Q2 | L2.5 exporte un journal de match sur l'appareil de l'hôte. Accord oral des deux joueurs, comme en P0 (n° 78) ? | Oui, hypothèse appliquée |
| Q3 | L2.6 (pages légales) est placé en P2 alors que les tests P2 se font entre proches. Le faire avant le premier test P2 ? | Oui : coût faible, et le lien peut circuler plus loin que prévu |
